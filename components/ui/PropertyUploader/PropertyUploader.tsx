...
function PropertyUploader({ amenities }) {
  ...
  const [uploadMutation] = useUploadPropertyMutation();
  ...
  const onSubmit = async ({
    ...
    try {
      ...
      await uploadMutation({ variables: res });
  ...
  const uploadImage = async (file): Promise<string> => {
    try {
      var imagesRef = ref(getStorage(firebase), `images/${v4()}`);
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <Grid container spacing={3}>
          <Grid
            ...
            lg={3}
          >
            <HottelyButton color="primary" type="sumbit">
              Save property
            </HottelyButton>
          </Grid>
        </Grid>
